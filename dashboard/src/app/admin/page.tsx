    ...
    <AppSidebar>
      <div className="p-4 lg:p-6 space-y-6 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Users', value: users.length, icon: Users, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-950/30' },
            { label: 'Active', value: users.filter(u => u.status === 'active').length, icon: UserCheck, color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-950/30' },
            { label: 'Suspended', value: users.filter(u => u.status === 'suspended').length, icon: UserX, color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-950/30' },
            { label: 'Admins', value: users.filter(u => u.role === 'admin').length, icon: Shield, color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-950/30' },
          ].map((card, i) => (
            <motion.div
              key={card.label}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            ...
            <option value="fleet-manager">Fleet Manager ({roleCounts['fleet-manager'] || 0})</option>
            <option value="operator">Operator ({roleCounts.operator || 0})</option>
              ...
              >
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2">
                    <span className={cn('px-2 py-0.5 rounded text-xs font-medium', rc.bg, rc.text)}>
                      {u.role}
                    </span>
                  </div>
                  {u.phone && (
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    <Activity className="w-3 h-3" /> Last login: {new Date(u.lastLogin).toLocaleDateString()}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-border">
                  ...
                  {canDelete && u.role !== 'admin' && (
                    <button className="px-3 py-1.5 rounded-lg border border-border text-xs font-medium text-destructive hover:bg-destructive/10 flex items-center gap-1 transition-colors">
