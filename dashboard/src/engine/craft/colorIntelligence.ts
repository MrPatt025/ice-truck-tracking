...
export class ColorIntelligenceEngine {
  ...
  adjustForSeverity(color: OKLCHColor, level?: AlertLevel): OKLCHColor {
    const factor = level
      ? SEVERITY_SATURATION[level]
      : this._config.saturationScale
    return {
      ...color,
  ...
  private _applyColorVars(): void {
    ...
    const accent = this.process(
      { l: 0.75, c: 0.15, h: 250, a: 1 },
    ...
    root.style.setProperty(
      '--ci-temperature',
      String(this._config.baseTemperature)
    )
    root.style.setProperty(
      '--ci-saturation-scale',
      String(this._config.saturationScale)
    )
    ...
    if (this._alertLevel) {
      ...
      const alertColor = this.process(
        { l: 0.7, c: 0.22, h: alertHue, a: 1 },
        this._alertLevel
      )
