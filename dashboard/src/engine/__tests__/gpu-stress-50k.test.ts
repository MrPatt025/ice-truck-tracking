...
describe('GPU Stress Test — 50,000 Entities', () => {
  ...
  test('GPU Memory Guard stays within budget at 50k entities', () => {
    ...
    for (const snap of history) {
      expect(snap.textureMemoryMB + snap.bufferMemoryMB).toBeLessThanOrEqual(
        512
      )
    }
  ...
  test('Context loss recovery under 50k entity load', () => {
    ...
    const unsubLost = pool.onLost('loss-test', () => {
      /* lostFired */
    })
    const unsubRestored = pool.onRestored('loss-test', () => {
      /* restoredFired */
    })
  ...
  test('Memory guard snapshot captures pool stats correctly', () => {
    ...
    expect(
      (snapshot?.textureMemoryMB ?? 0) + (snapshot?.bufferMemoryMB ?? 0)
