...
export function useAppHealthEvents(): AppHealthState {
  ...
  >('unknown')
  const [wsDetail, setWsDetail] = React.useState<WsHealthEventDetail | null>(
    null
  )
  const [backendDetail, setBackendDetail] =
    React.useState<BackendHealthEventDetail | null>(null)
