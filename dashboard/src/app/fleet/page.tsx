...
function generateMockTrucks(count: number): TruckResponse[] {
  const statuses: TruckResponse['status'][] = ['active', 'idle', 'maintenance', 'offline', 'alert'];
  const drivers = ['Somchai K.', 'Prasert W.', 'Nattapong S.', 'Kittisak P.', 'Wichai M.', 'Anong T.', 'Piyapong C.', 'Surasak L.'];
  const routes = ['Bangkok → Chiang Mai', 'Phuket → Surat Thani', 'Nonthaburi Loop', 'Eastern Seaboard', 'Isaan Ring'];

  return Array.from({ length: count }, (_, i) => ({
    id: `truck-${String(i + 1).padStart(3, '0')}`,
    name: `ICE-${String(i + 1).padStart(3, '0')}`,
    plateNumber: `${String.fromCharCode(65 + (i % 26))}${String.fromCharCode(65 + ((i * 7) % 26))}-${1000 + i}`,
    status: statuses[i % statuses.length],
    driver: drivers[i % drivers.length],
    temperature: -18 + Math.random() * 10,
    humidity: 30 + Math.random() * 40,
    speed: statuses[i % statuses.length] === 'active' ? 40 + Math.random() * 60 : 0,
...
type SortKey = 'name' | 'status' | 'temperature' | 'speed' | 'fuelLevel' | 'driver' | 'lastUpdate';
    ...
    <AppSidebar>
      <div className="p-4 lg:p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          {(['all', 'active', 'idle', 'maintenance', 'offline', 'alert'] as const).map(status => (
            <button
              key={status}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {paged.map((truck) => {
                  const statusStyle = truckStatusColors[truck.status] || truckStatusColors.offline;
                  return (
                    <motion.tr
                      key={truck.id}
                      initial={{ opacity: 0 }}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{truck.driver}</td>
                      <td className="px-4 py-3">
                        <span className={cn('font-mono', truck.temperature > -10 ? 'text-amber-500' : 'text-blue-500')}>
                          {truck.temperature.toFixed(1)}°C
                        </span>
                      </td>
                      <td className="px-4 py-3 font-mono">{truck.speed.toFixed(0)} km/h</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 rounded-full bg-muted overflow-hidden">
                            <div
                              className={cn('h-full rounded-full', truck.fuelLevel > 30 ? 'bg-green-500' : 'bg-red-500')}
                              style={{ width: `${truck.fuelLevel}%` }}
                            />
                          </div>
                          <span className="text-xs text-muted-foreground">{truck.fuelLevel.toFixed(0)}%</span>
                        </div>
                      </td>
                ...
                return (
                  <div className="bg-card rounded-xl border border-border p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold">{t.name} — {t.plateNumber}</h3>
                      <button
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                      {[
                        { icon: MapPin, label: 'Route', value: t.route || '—', color: 'text-blue-500' },
                        { icon: Thermometer, label: 'Temperature', value: `${t.temperature.toFixed(1)}°C`, color: 'text-cyan-500' },
                        { icon: Gauge, label: 'Speed', value: `${t.speed.toFixed(0)} km/h`, color: 'text-amber-500' },
                        { icon: Fuel, label: 'Fuel', value: `${t.fuelLevel.toFixed(0)}%`, color: 'text-green-500' },
                        { icon: Battery, label: 'Battery', value: `${t.batteryLevel.toFixed(0)}%`, color: 'text-purple-500' },
                        { icon: Truck, label: 'Deliveries', value: String(t.deliveries), color: 'text-primary' },
