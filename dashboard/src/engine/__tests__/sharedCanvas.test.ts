...
describe('SharedCanvasPool', () => {
  ...
  test('acquire creates a canvas with the right dimensions', () => {
    ...
    const { canvas, gl, version } = pool.acquire('test-1', {
      width: 800,
      height: 600,
    })
  ...
  test('onLost / onRestored fire callbacks', () => {
    ...
    if (lostHandlers) {
      for (const h of lostHandlers)
        (h as EventListener)(new Event('webglcontextlost'))
    }
    ...
    if (restoredHandlers) {
      for (const h of restoredHandlers)
        (h as EventListener)(new Event('webglcontextrestored'))
    }
