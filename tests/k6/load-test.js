...
/* ─── Scenarios ─── */
export default function vuFunction(data) {
  // NOSONAR — k6 VU function receives setup() data
  const headers = {
  ...
  group('Telemetry Ingestion', () => {
    const seed = Math.trunc(
      ((__VU + 1) * 0x45d9f3b) ^ ((__ITER + 1) * 0x9e3779b9) ^ 0x7f4a7c15
    )
