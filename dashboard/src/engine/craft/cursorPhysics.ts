...
const MODE_STYLES: Record<
  CursorMode,
  { scale: number; borderRadius: string; color: string }
> = {
  default: {
    scale: 1,
    borderRadius: '50%',
    color: 'oklch(0.85 0.08 250 / 0.5)',
  },
  map: { scale: 1.5, borderRadius: '50%', color: 'oklch(0.80 0.12 200 / 0.4)' },
  panel: {
    scale: 0.8,
    borderRadius: '4px',
    color: 'oklch(0.90 0.06 260 / 0.6)',
  },
  drag: {
    scale: 1.3,
    borderRadius: '50%',
    color: 'oklch(0.75 0.10 140 / 0.5)',
  },
  hover: {
    scale: 1.6,
    borderRadius: '50%',
    color: 'oklch(0.88 0.15 300 / 0.3)',
  },
  click: {
    scale: 0.7,
    borderRadius: '50%',
    color: 'oklch(0.95 0.20 200 / 0.7)',
  },
}
...
export class CursorPhysicsEngine {
  ...
  mount(): void {
    ...
    Object.assign(this._el.style, {
      ...
      transition:
        'width 200ms cubic-bezier(0.22,1,0.36,1), height 200ms cubic-bezier(0.22,1,0.36,1), border-radius 200ms cubic-bezier(0.22,1,0.36,1)',
    ...
    Object.assign(this._glowEl.style, {
      ...
      borderRadius: '50%',
      background:
        'radial-gradient(circle, oklch(0.80 0.12 250 / 0.15) 0%, transparent 70%)',
      pointerEvents: 'none',
      zIndex: '99998',
    ...
    for (let i = 0; i < this._config.glowTrailLength; i++) {
      ...
      const trailSize =
        this._config.size * (1 - i / this._config.glowTrailLength) * 0.6
  ...
  private _updatePhysics(_dt: number): void {
    ...
    this._magnetTargets.forEach(el => {
      ...
      if (dist < this._config.magneticRadius && dist > 0) {
        const force =
          (1 - dist / this._config.magneticRadius) *
          this._config.magneticStrength
  ...
  private _render(): void {
    ...
    for (let i = 0; i < this._trailEls.length; i++) {
      ...
      if (point) {
        this._trailEls[i].style.transform =
