...
function getTruckColor(truck: TruckTelemetry): RGBA {
  if (
    truck.temperature > TEMP_HIGH_LIMIT ||
    truck.temperature < TEMP_LOW_LIMIT
  ) {
...
function resolveTelemetryTimestampMs(
...
function createTruckPopupContent(truck: TruckRenderPoint): HTMLElement {
  ...
  ;(card.style as unknown as Record<string, string>).webkitBackdropFilter =
    'blur(12px)'
  ...
  card.style.transition =
    'opacity 170ms ease-out, transform 220ms cubic-bezier(0.22, 0.61, 0.36, 1)'
...
export class ImperativeMapLayer {
  ...
  private readonly mapErrorHandler = (event: mapboxgl.ErrorEvent): void => {
    ...
    const errorLike = event.error as
    ...
    console.warn(
      'Mapbox style request unauthorized. Falling back to open map style.'
    )
  ...
  init(container: HTMLElement, style: Theme = 'dark'): void {
    ...
    if (!E2E_LIGHT_MODE && MAPBOX_TOKEN) {
      ...
      console.warn(
        'NEXT_PUBLIC_MAPBOX_TOKEN missing. Using public open style fallback.'
      )
    }
  ...
  private installDeckOverlay(): void {
    ...
    if (!canvas?.style) {
      ...
      this.overlay = null
      console.warn(
        'Deck overlay disabled because map canvas is not ready for touchAction binding.'
      )
      return
    }
    if (this.overlay) {
    ...
    try {
      ...
      if (message.toLowerCase().includes('touchaction')) {
        ...
        this.overlay = null
        console.warn(
          'Deck overlay disabled due to TouchAction runtime mismatch.'
        )
        return
      }
      throw error
  ...
  private buildSelectedGlowLayer(
    nowMs: number
  ): ScatterplotLayer<TruckRenderPoint> | null {
  ...
  private buildClusterLayer(): ScreenGridLayer<TruckRenderPoint> {
    return new ScreenGridLayer<TruckRenderPoint>({
      ...
      getWeight: d =>
        1 +
        clampUnit(d.speed / 120) +
        clampUnit(Math.abs(d.temperature + 12) / 20),
      colorRange: [
        [8, 47, 73, 70],
        [14, 116, 144, 92],
        [6, 182, 212, 116],
  ...
  private loadCachedFleetIfNeeded(): void {
    ...
    void readFleetSnapshot().then(cached => {
      ...
      if (
        liveTrucks.size > 0 ||
        (connectionStatus === 'connected' && !browserOffline)
      ) {
        return
      }
  ...
  private persistLiveFleetSnapshot(
    liveTrucks: ReadonlyMap<string, TruckTelemetry>,
    now: number
  ): void {
  ...
  private trimHeatmapTrail(now: number): void {
    while (this.heatmapTrail.length > 0) {
      ...
      if (
        now - head.at <= HEATMAP_POINT_TTL_MS &&
        this.heatmapTrail.length <= HEATMAP_HISTORY_LIMIT
      ) {
        break
      }
  ...
  private updateHeatmapFade(): void {
    ...
    if (this.modeBlend !== this.modeTransitionTo) {
      ...
      const rawProgress = Math.max(
        0,
        Math.min(1, elapsed / MODE_TRANSITION_DURATION_MS)
      )
      const easedProgress = rawProgress * rawProgress * (3 - 2 * rawProgress)
      this.modeBlend =
        this.modeTransitionFrom +
  ...
  private syncCinematicModeBlend(): void {
    ...
    const targetMode: 'live' | 'historical' =
      this.modeTransitionTo >= 0.5 ? 'historical' : 'live'
  ...
  private syncFromTransientStore(): void {
    ...
    trucks.forEach(truck => {
      ...
      if (existingPoint) {
        ...
        if (moved) {
          ...
          existingPoint.lerpDurationMs = Math.min(
            ...
            Math.max(
              ...
              (packetTimestamp - existingPoint.lastPacketAt) * 0.9 ||
                LERP_BASE_DURATION_MS
            )
          )
        }
        ...
        this.pushHeatmapPoint(
          nextPosition,
          resolveHeatmapWeight(truck.speed, truck.temperature, truck.status),
          now
        )
      ...
      const initialStaleFactor = resolveStaleFactor(
        now - initialPacketTimestamp
      )
  ...
  private refreshDeckLayer(): void {
    ...
    if (
      glowLayer &&
      this.liveVisibility > OPACITY_EPSILON &&
      !useClusterLayer
    ) {
  ...
  private selectTruckForCinematicView(
    truckId: string,
    latitude: number,
    longitude: number
  ): void {
