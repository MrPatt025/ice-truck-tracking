...
module.exports = {
  ...
  'dashboard/**/*.{js,jsx,ts,tsx}': (files) => {
    ...
    const configFiles = files.filter(f => f.endsWith('.config.ts') || f.endsWith('.config.js') || f.endsWith('.config.mjs'));
    const sourceFiles = files.filter(f => !(f.endsWith('.config.ts') || f.endsWith('.config.js') || f.endsWith('.config.mjs')));
    const commands = [];
    if (sourceFiles.length > 0) {
      commands.push(`eslint --fix --cache --cache-location .eslintcache ${sourceFiles.join(' ')}`);
    }
    return commands;
  },
