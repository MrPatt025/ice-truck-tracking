...
describe('Observability — EnvSnapshot Contract', () => {
  it('environment snapshot includes all monitoring dimensions', () => {
    ...
    expect(mockEnv.memoryPressure).toBeCloseTo(
      mockEnv.heapUsedMB / mockEnv.heapLimitMB,
      1
    )
...
describe('Observability — Performance Metrics Contract', () => {
  ...
  it('fleet metrics contain all operational dimensions', () => {
    ...
    expect(
      mockFleet.activeTrucks + mockFleet.idleTrucks + mockFleet.offlineTrucks
    ).toBe(10_000)
    expect(
      mockFleet.criticalAlerts + mockFleet.warningAlerts
    ).toBeLessThanOrEqual(mockFleet.totalAlerts)
    expect(mockFleet.minTemperature).toBeLessThanOrEqual(
      mockFleet.avgTemperature
    )
    expect(mockFleet.avgTemperature).toBeLessThanOrEqual(
      mockFleet.maxTemperature
    )
  })
})
