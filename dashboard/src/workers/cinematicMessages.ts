...
export type CinematicTransitionPhase = 'idle' | 'intro' | 'outro' | 'handoff'
...
function isTelemetryPayload(
  value: unknown
): value is CinematicTelemetryPayload {
  ...
  if (fleet !== undefined) {
    ...
    const hasInvalidNode = fleet.some(node => {
      ...
      if (!isFiniteNumber(node.latitude) || !isFiniteNumber(node.longitude))
        return true
      if (!isFiniteNumber(node.heading) || !isFiniteNumber(node.tempC))
        return true
      return (
        node.status !== 'active' &&
        node.status !== 'idle' &&
...
function isTransitionPayload(
  value: unknown
): value is CinematicTransitionPayload {
  if (!isRecord(value)) return false
  return (
...
function isCameraFlyToPayload(
  value: unknown
): value is CinematicCameraFlyToPayload {
  if (!isRecord(value)) return false
