...
export class MotionCalibrationLab {
  ...
  getTiming(intent: MotionIntent): CalibratedTiming {
    ...
    return {
      ...
      anticipation: {
        ...timing.anticipation,
        duration: timing.anticipation.duration * this._globalScale,
      },
      action: {
        ...timing.action,
        duration: timing.action.duration * this._globalScale,
      },
      settling: {
        ...timing.settling,
        duration: timing.settling.duration * this._globalScale,
      },
  ...
  cssKeyframes(intent: MotionIntent, name: string): string {
    ...
    const actionEnd =
      ((t.anticipation.duration + t.action.duration) / t.totalMs) * 100
