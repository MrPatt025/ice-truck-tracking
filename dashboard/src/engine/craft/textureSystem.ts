...
export class TextureCompositor {
  ...
  mount(parent?: HTMLElement): void {
    ...
    this._micro.mount(p, {
      ...config.microGrain,
      opacity: config.microGrain.opacity * scale,
    })
    this._metal.mount(p, {
      ...config.brushedMetal,
      opacity: config.brushedMetal.opacity * scale,
    })
    this._dust.mount(p, {
      ...config.dustGrain,
      opacity: config.dustGrain.opacity * scale,
    })
    this._fabric.mount(p, {
      ...config.fabric,
      threadOpacity: config.fabric.threadOpacity * scale,
    })
  }
