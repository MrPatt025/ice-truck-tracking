...
export class ClientObservability {
  ...
  private observeErrors(): void {
    ...
    globalThis.window.addEventListener('error', event => {
      this.recordError(event.error ?? new Error(event.message), 'window')
    })
  ...
  private flush(): void {
    ...
    if (vitals.length === 0 && engine.length === 0 && errors.length === 0)
      return
