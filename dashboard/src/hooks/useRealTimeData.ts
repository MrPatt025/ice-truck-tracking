...
export function useRealTimeData() {
      ...
      try {
        ...
        ws.onclose = () => {
          ...
          if (retryRef.current < maxRetry) {
            ...
            retryRef.current += 1
            reconnectTimeoutRef.current = globalThis.window.setTimeout(
              connect,
              wait
            )
          }
    ...
    const poll = async () => {
      try {
        const [tRes, aRes] = await Promise.all([
          fetch(`${API_BASE}/trucks`, { signal: abortController.signal }).catch(
            () => null
          ),
          fetch(`${API_BASE}/alerts`, { signal: abortController.signal }).catch(
            () => null
          ),
        ])
        if (aborted) return
      ...
      if (!isConnected && !aborted)
        tm = globalThis.window.setTimeout(poll, 10000)
    }
    if (!isConnected) poll()
    return () => {
      aborted = true
  ...
  return {
    ...
    alerts,
    isConnected,
    setTrucks: setTrucksState,
    setAlerts: setAlertsState,
  }
}
