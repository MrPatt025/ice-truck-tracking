...
const GlassCard = memo(
  ({
    ...
    >
      <div
        className={`relative rounded-3xl bg-slate-900/85 backdrop-blur-2xl ring-1 ring-white/10 ${className}`}
      >
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Optimization: Load THREE.js dynamically and only once
  ...
  }, [autoRefresh, refreshSpeed, simulationSpeed, paused, flags]);
  ...
  const dataPoints = useMemo(() => {
    switch (timeRange) {
      ...
      case '30d':
        return dataPrecision === 'high' ? 120 : 60;
      case '90d':
        return dataPrecision === 'high' ? 180 : 90;
      default:
        return 24;
    }
  ...
  const systemHealth = useMemo(() => {
    const _apiStatus = healthOnline
      ...
      : healthOnline === false
        ? 'Offline'
        : 'Checking';
    const gpsStatus = trucks.length > 0;
  ...
  }, [healthOnline, trucks.length, lastDataTimestamp]);
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      {
        title: 'Anomaly Index',
        value: computedMetrics.anomalyIndex,
        change:
    ...
    <div className="relative min-h-screen overflow-x-hidden text-white selection:bg-violet-500/30 selection:text-white">
      ...
      {flags['developer-mode'] && (
        <div className="fixed top-2 left-2 z-[100] p-3 rounded-lg bg-black/70 backdrop-blur-sm ring-1 ring-amber-400/50 text-amber-300 text-xs font-mono shadow-glow">
          <Code className="inline-block h-3 w-3 mr-1" /> DEVELOPER MODE (Ctrl+D)
          <p className="mt-1">
            Latency:{' '}
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-900/90 backdrop-blur-2xl shadow-lg shadow-black/30">
        <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-4 px-6 py-4 flex-wrap">
          <div className="flex items-center gap-4">
            ...
            {/* Title */}
            <div>
              <p className="text-[9px] uppercase tracking-[.4em] text-slate-400">
        ...
        <div className="border-t border-white/5 bg-slate-950/50 backdrop-blur-xl">
          <div className="mx-auto max-w-[1800px] px-6 py-3 flex items-center justify-between gap-4 flex-wrap text-xs">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-slate-400">Data Detail:</span>
                <select
                  value={dataPrecision}
                  onChange={(e) =>
                ...
                <button
                  ...
                  className={`rounded-xl px-2 py-1 text-xs font-semibold ring-1 transition-all ${
                    flags['anomaly-index']
                      ...
                      : 'bg-white/10 text-slate-400 ring-white/15'
                  }`}
                >
      ...
      <main className="mx-auto max-w-[1800px] space-y-8 px-6 py-8">
              ...
              <div className="mt-6 space-y-3">
                {tempBuckets.map((b, i) => (
                  ...
                  >
                    ...
                    <span className="font-bold text-lg">{b.value} Trucks</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="absolute inset-0 flex items-center justify-center p-6">
                  <div className="w-full text-center space-y-4">
                    ...
                    <p className="text-3xl font-bold">
                      {trucks.length} Active Telemetry Streams
                    </p>
                      ...
                      {trucks.slice(0, 8).map((t) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4 text-slate-400">
                            ...
                            <span>⏱ {t.speed.toFixed(0)} km/h</span>
                            <span>🌡 {t.temp.toFixed(1)}°C</span>
                          </div>
                        </li>
                      ))}
                      {trucks.length > 8 && (
                        <li className="text-center text-sm text-slate-500 pt-2">
                          ... and {trucks.length - 8} more trucks
                        </li>
                      )}
                      {trucks.length === 0 && (
        ...
        <div className="text-center space-y-2 pt-4 pb-8">
          <div className="flex items-center justify-center gap-4 flex-wrap text-xs text-slate-500">
            ...
            <span>•</span>
            <span>
              Version: <span className="font-mono text-slate-400">v2.1.0</span>
            </span>
            <span>•</span>
            <span>
