...
const OfflineBanner = memo(function OfflineBanner({
  ...
  return (
    <div
      ...
      className={`glass-panel isolate z-[60] min-h-[4.5rem] rounded-xl border border-amber-200/30 bg-amber-100/10 p-3 text-amber-100 shadow-xl transition-[opacity,transform] duration-200 ${isOffline ? 'relative block opacity-100 translate-y-0' : 'pointer-events-none invisible absolute opacity-0 -translate-y-2'} ${className}`}
    >
