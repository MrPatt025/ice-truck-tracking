...
export function MapView({ trucks, selectedTruck, onSelectTruck, geofences = [], className }: MapViewProps) {
  ...
  const clusters = useMemo(() => {
    ...
    trucks.forEach(truck => {
      ...
      const nearbyTrucks = trucks.filter(otherTruck => {
        if (processed.has(otherTruck.id) || truck.id === otherTruck.id) return false
      ...
      if (nearbyTrucks.length > 0) {
        ...
        const avgLat = clusterTrucks.reduce((sum, t) => sum + t.latitude, 0) / clusterTrucks.length
        const avgLng = clusterTrucks.reduce((sum, t) => sum + t.longitude, 0) / clusterTrucks.length
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      ...
      >
        {/* Individual Trucks */}
        {(!showClusters ? trucks : trucks.filter(truck => 
          !clusters.some(cluster => cluster.trucks.some(t => t.id === truck.id))
        )).map(truck => {
          ...
          return (
            <Tooltip key={truck.id} content={`${truck.driver_name} - ${truck.status}`}>
              <div
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 ${getStatusColor(truck.status)} ${
                ...
                {isSelected && (
                  <Card className="absolute top-8 left-1/2 transform -translate-x-1/2 min-w-48 z-30">
                    <CardContent padding="sm">
                      <h3 className="font-semibold text-sm mb-2">{truck.driver_name}</h3>
                      <div className="space-y-1 text-xs">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-blue-600">{truck.temperature}°C</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Status:</span>
                          <span className={`font-medium ${
                            truck.status === 'active' ? 'text-green-600' : 
                            truck.status === 'maintenance' ? 'text-orange-600' : 'text-gray-600'
                          }`}>
                            {truck.status}
                          </span>
                        </div>
        ...
        {showClusters && clusters.map(cluster => {
          ...
          return (
            <Tooltip key={cluster.id} content={`${cluster.count} trucks in this area`}>
              <div
      ...
      <div className="sr-only" aria-live="polite" aria-atomic="true">
        ...
        {showClusters && `Clustering enabled, showing ${clusters.length} clusters`}
