...
export class ParticleMicroSystem {
  ...
  emitSparks(x: number, y: number, count = 8): void {
    for (
      let i = 0;
      i < count && this._particles.length < this._config.maxParticles;
      i++
    ) {
  ...
  private _update(dt: number): void {
    ...
    const ambientCount = this._particles.filter(
      p => p.type === 'ambient'
    ).length
    const targetAmbient = Math.floor(
      this._config.ambientCount * this._currentScale
    )
  ...
  private _applyParticleDecay(particle: Particle, dt: number): void {
    ...
    particle.opacity = Math.max(
      0,
      particle.life * (particle.type === 'ambient' ? 0.15 : 0.6)
    )
  }
  ...
  private _autoScale(): void {
    ...
    if (avgFps < this._config.fpsThreshold && this._currentScale > 0.1) {
      ...
      avgFps > this._config.fpsThreshold + 5 &&
