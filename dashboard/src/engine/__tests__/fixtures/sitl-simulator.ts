...
export class SITLSimulator {
  ...
  private initTrucks(): void {
    const { truckCount, areaCenter, areaRadiusKm, temperatureRange } =
      this.config
    ...
    for (let i = 0; i < truckCount; i++) {
      ...
      this.trucks.push({
        ...
        driverName:
          DRIVER_NAMES[Math.floor(secureRandomFloat() * DRIVER_NAMES.length)],
  ...
  private updateGPSPosition(
    ...
    const dist = Math.hypot(
      truck.lat - truck._targetLat,
      truck.lng - truck._targetLng
    )
    if (dist < 0.01) {
    ...
    truck.heading =
      (truck.heading + (secureRandomFloat() - 0.5) * 15 + 360) % 360
  }
  ...
  private updateTemperature(truck: TruckState): void {
    ...
    if (truck._spiking) {
      ...
      if (truck._spikeCountdown <= 0) {
        ...
        truck.temperature = randomInRange(
  ...
  tick(): TruckTelemetry[] {
    ...
    for (const truck of this.trucks) {
      ...
      truck.fuelLevel = clamp(
        truck.fuelLevel - secureRandomFloat() * 0.05,
        0,
        100
      )
      ...
      truck.engineRpm =
        truck.speed > 0
          ? clamp(
              800 + truck.speed * 30 + (secureRandomFloat() - 0.5) * 200,
              800,
              6000
            )
          : 800
  ...
  get stats() {
    return {
      ...
      dropRate:
        this._totalPackets > 0 ? this._droppedPackets / this._totalPackets : 0,
