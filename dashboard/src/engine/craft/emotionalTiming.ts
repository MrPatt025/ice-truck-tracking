...
export class EmotionalTimingEngine {
  ...
  private readonly _activeAnimations = new Map<
    string,
    { raf: number; startTime: number }
  >()
  ...
  toKeyframes(tone: EmotionalTone, name: string): string {
    ...
    const actEnd =
      ((seq.anticipation.duration + seq.action.duration) / seq.totalMs) * 100
  ...
  validate(seq: EmotionalSequence): { valid: boolean; issues: string[] } {
    ...
    if (seq.anticipation.duration <= 0 && seq.settling.duration <= 0) {
      issues.push(
        'Missing both anticipation and settling phases — motion will feel mechanical'
      )
    }
    if (seq.totalMs > 1200) {
    ...
    if (seq.settling.duration < seq.anticipation.duration * 0.5) {
      issues.push(
        'Settling phase too short relative to anticipation — motion will feel abrupt'
      )
    }
