...
async function runSoakLoop(params: {
  ...
  while (Date.now() - startTime < durationMs) {
    ...
    try {
      ...
      errors++
      failures.push(
        `Tick ${tick}: ${err instanceof Error ? err.message : String(err)}`
      )
    }
...
async function main() {
  ...
  const sorted = (tickTimings.length > 0 ? [...tickTimings] : [0]).sort(
    (a, b) => a - b
  )
  ...
  const report: ImmersionReport = {
    ...
    status:
      soak.errors === 0 &&
      soak.memoryPeak < (startMem.heapUsed / 1024 / 1024) * 3
        ? 'pass'
        : 'fail',
