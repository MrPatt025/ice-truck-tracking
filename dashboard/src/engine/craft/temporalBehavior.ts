...
export class TemporalUIEngine {
  ...
  // Callbacks
  private _onTimeShift:
    | ((segment: TimeSegment, temperature: number) => void)
    | null = null
  ...
  constructor(config?: Partial<TemporalConfig>) {
    ...
    this._state = {
      ...
      isIdle: false,
      isPeakHours:
        hour >= this._config.peakHoursStart && hour < this._config.peakHoursEnd,
      trafficLoad: 0,
  ...
  mount(): void {
    ...
    globalThis.window?.addEventListener('mousemove', this._activityHandler, {
      passive: true,
    })
    globalThis.window?.addEventListener('keydown', this._activityHandler, {
      passive: true,
    })
    globalThis.window?.addEventListener('scroll', this._activityHandler, {
      passive: true,
    })
    globalThis.window?.addEventListener('touchstart', this._activityHandler, {
      passive: true,
    })
  ...
  private _applyCSSVars(): void {
    ...
    root.style.setProperty(
      ...
      String(SEGMENT_TEMPERATURE[this._state.segment])
    )
    root.style.setProperty(
      '--temporal-ambient',
      String(this._state.ambientIntensity)
    )
    root.style.setProperty('--temporal-idle', this._state.isIdle ? '1' : '0')
    root.style.setProperty(
      '--temporal-peak',
      this._state.isPeakHours ? '1' : '0'
    )
    root.style.setProperty(
      '--temporal-traffic',
      String(this._state.trafficLoad)
    )
