...
class AuthService {
  ...
  async refreshToken(oldRefreshToken) {
    ...
    if (stored.revoked_at) {
      ...
      logger.warn(
        { family: stored.family, userId: stored.user_id },
      ...
      throw Object.assign(new Error('Token reuse detected — all sessions revoked'), {
        statusCode: 401,
      });
    }
    ...
    const { accessToken, refreshToken: newRefreshToken } = await this._createTokenPair(
      user,
      stored.family
    );
  ...
  _getPermissions(role) {
    const permissions = {
      ...
      driver: ['trucks:read', 'tracking:read', 'tracking:write', 'alerts:read', 'routes:read'],
