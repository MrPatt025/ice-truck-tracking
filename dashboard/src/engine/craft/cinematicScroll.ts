...
export class CinematicScrollEngine {
  ...
  // Callbacks
  private _onProgress: ((progress: number, section: string) => void) | null =
    null
  private _onSectionChange:
    | ((sectionId: string, direction: 'up' | 'down') => void)
    | null = null
  private _onShaderUpdate:
    | ((progress: number, velocity: number) => void)
    | null = null
  ...
  onSectionChange(
    cb: (sectionId: string, direction: 'up' | 'down') => void
  ): void {
  ...
  private _updateState(dt: number): void {
    ...
    if (this._config.inertia) {
      this._state.velocity =
        this._state.velocity * this._config.inertiaDamping +
        rawVelocity * (1 - this._config.inertiaDamping)
    } else {
    ...
    // Global progress
    const maxScroll =
      document.documentElement.scrollHeight - browserWindow.innerHeight
    ...
    document.documentElement.style.setProperty(
      ...
      String(this._state.progress)
    )
    document.documentElement.style.setProperty(
      '--craft-scroll-velocity',
      String(Math.abs(this._state.velocity) / 1000)
    )
  }
  ...
  private _updateSections(): void {
    ...
    for (const section of this._sections) {
      ...
      section.el.style.setProperty(
        '--craft-section-progress',
    ...
    if (newActiveSection !== this._state.activeSection) {
      const dir =
        this._state.direction === 'idle' ? 'down' : this._state.direction
    ...
    // Snap logic
    if (
      this._config.snapEnabled &&
      this._state.direction === 'idle' &&
      Math.abs(this._state.velocity) < 2
    ) {
      this._checkSnap()
    }
  }
