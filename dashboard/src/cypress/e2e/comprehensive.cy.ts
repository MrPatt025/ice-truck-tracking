...
describe('Ice Truck Tracking Dashboard - Comprehensive E2E Tests', () => {
	...
	describe('Real-time Map Functionality', () => {
		...
		it('should update truck positions in real-time', () => {
			cy.get('[data-testid="truck-marker"]')
				.first()
				.then($marker => {
					const marker = $marker as { position: () => unknown }
					...
					cy.get('[data-testid="truck-marker"]')
						.first()
						.then($updatedMarker => {
							const updatedMarkerEntry = $updatedMarker as {
								position: () => unknown
							}
							const updatedPosition = updatedMarkerEntry.position()
		...
		it('should handle map interactions', () => {
			...
			cy.get('[data-testid="map-container"]').trigger('mousedown', {
				button: 0,
			})
			cy.get('[data-testid="map-container"]').trigger('mousemove', {
				clientX: 100,
				clientY: 100,
			})
