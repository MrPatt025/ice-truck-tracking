...
function startSimulation(): void {
  ...
  simInterval = setInterval(() => {
    ...
    trucks.forEach(t => {
      ...
      t.fuelLevel = Math.max(
        5,
        Math.min(100, t.fuelLevel - secureRandomRange(0, 0.05))
      )
...
function applyDirectTruckPayload(payload: Record<string, unknown>): void {
  if (
    !(payload.id || payload.truckId || payload.truck_id) ||
...
function handleTruckStatusUpdate(raw: unknown): void {
  ...
  const id =
    ...
    (payload.truckId as string) ||
    (payload.truck_id as string)
  if (!id) return
...
function emitChartPoints(): void {
  ...
  const chartPoints: Array<{ series: string; point: TimeSeriesPoint }> = [
    {
      series: 'temperature',
      point: { timestamp: now, value: m.avgTemperature },
    },
    { series: 'speed', point: { timestamp: now, value: m.avgSpeed } },
    { series: 'fuel', point: { timestamp: now, value: m.avgFuelLevel } },
    {
      series: 'active-trucks',
      point: { timestamp: now, value: m.activeTrucks },
    },
