...
export class Spring2D {
  ...
  constructor(initialX = 0, initialY = 0, config: Partial<SpringConfig> = {}) {
...
export class VelocityTracker {
  ...
  addSample(x: number, y: number, timestamp = performance.now()): void {
    ...
    while (
      ...
      (this.samples.length > 2 && this.samples[0].t < cutoff)
    ) {
      this.samples.shift()
    }
  }
...
export class GestureEngine {
  ...
  /** Set update callback */
  onChange(cb: (state: GestureState) => void): void {
    this.onGestureUpdate = cb
  }
